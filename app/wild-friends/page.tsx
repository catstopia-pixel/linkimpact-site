...
export default async function WildFriendsPage({searchParams}:{searchParams:Promise<{lang?:string}>}) {
  ...
  return <main className="min-h-screen bg-[#f7f5e9] text-[#153a32]">
    ...
    <section className="bg-[#f4c968] text-[#153a32]"><div className="mx-auto max-w-5xl px-5 py-20 text-center md:py-28"><h2 className="text-4xl font-black md:text-6xl">{t.finalTitle}</h2><p className="mx-auto mt-7 max-w-3xl text-lg leading-8">{t.finalBody}</p><p className="mt-5 text-2xl font-black">{t.finalLine}</p><div className="mx-auto mt-10 max-w-xl rounded-[2rem] bg-white p-8 shadow-xl"><div className="text-xs font-black tracking-[.18em] text-[#43835a]">{t.formTitle}</div><p className="mt-4 leading-7 text-[#5d6c64]">{t.formBody}</p><div className="mt-6 cursor-not-allowed rounded-2xl bg-[#153a32] px-6 py-4 font-black text-white">{t.cta}<span className="mt-1 block text-xs font-bold text-white/60">{t.ctaPending}</span></div></div><div className="mx-auto mt-8 max-w-xl rounded-2xl border-2 border-[#153a32]/15 p-6"><b>{t.accepted}</b><p className="mt-2 text-sm leading-6">{t.acceptedBody}</p></div></div></section>
  </main>
}
