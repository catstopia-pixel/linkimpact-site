...
export default function OtterClapGame() {
  ...
  return (
    <div className="overflow-hidden rounded-[32px] bg-[#062e29] text-white shadow-2xl">
      <div className="relative min-h-[620px] select-none">
        <div className="absolute inset-0 bg-[radial-gradient(circle_at_50%_30%,#237566_0%,#0b433a_35%,#062e29_72%)]" />
        <div className="relative z-10 flex min-h-[620px] flex-col p-6 sm:p-9">
          <div className="flex items-center justify-between text-xs font-bold tracking-[.18em] text-emerald-100/80">
            <span>WILD LINK · SOVAC 2026</span><span>LINKIMPACT</span>
          </div>

          {phase === "intro" && <div className="my-auto text-center">
            <div className="mx-auto mb-6 grid h-40 w-40 place-items-center rounded-full border border-white/20 bg-white/10 text-8xl shadow-inner">🦦</div>
            ...
            <p className="mx-auto mt-5 max-w-md text-base leading-7 text-emerald-50/80">10초 동안 화면을 빠르게 터치해 수달에게 박수를 보내보세요. 게임이 끝나면 수달과 우리의 연결이 열립니다.</p>
          ...
          {phase === "play" && <button onPointerDown={clap} className="my-auto flex flex-1 touch-manipulation flex-col items-center justify-center outline-none">
            <div className="mb-5 flex w-full max-w-md items-center justify-between text-sm font-bold"><span>남은 시간 {timeLeft.toFixed(1)}s</span><span>박수 {score}</span></div>
            ...
            <div className={`mt-10 grid h-56 w-56 place-items-center rounded-full border-4 border-white/20 bg-white/10 text-[9rem] transition-transform ${pulse ? "scale-110 rotate-3" : "scale-100"}`}>🦦</div>
            <div className={`mt-7 text-5xl font-black ${pulse ? "scale-110" : ""}`}>👏 {score}</div>
            <p className="mt-3 text-sm text-emerald-100/70">화면 어디든 빠르게 터치!</p>
          </button>}
